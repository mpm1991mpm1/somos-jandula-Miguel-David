...
// Watch for changes in modelValue and update valorSeleccionado accordingly
watch(() => props.modelValue, (nuevoValor) => {
  selectedValue.value = nuevoValor;
...
const emitSelectUpdate = () => {
  ...
  console.log("Emitiendo planta:" + selectedValue.value);
};
</script>
...
<template>
  <div>
    ...
    <select 
      ...
      <option value="default">Seleccione una planta.</option>

      <!-- Generate the options dynamically from props -->
      <option 
        v-for="option in props.optionsList" 
        :key="option.floorName" 
        :value="option.floorName">
        {{ option.floorName }}
      </option>
